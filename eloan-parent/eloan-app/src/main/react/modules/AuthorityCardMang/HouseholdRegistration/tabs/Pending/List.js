...
export default React.createClass({
  ...
  fetch(params = {}) {
    ...
    Utils.ajaxData({
      url: '/modules/workflow/ProcessTaskController/queryHouseholdInvestigate.htm?isCompleted=false',
      data: params,
      callback: (result) => {
    ...
    return (
      <div className="block-panel">
        <div className="actionBtns" style={{ marginBottom: 16 }}>
          <button className="ant-btn"  disabled={!hasSelected} onClick={this.showModal.bind(this, '下户登记', true) }>
            下户登记
          </button>
        </div>
        ...
        <AddlWin ref="AddlWin"
          visible={state.visible} formData={state.formData}
          title={state.title} hideModal={me.hideModal} id={state.id} record={state.selectedrecord} canEdit={state.canEdit}/>
      </div>
    );
  }
})
