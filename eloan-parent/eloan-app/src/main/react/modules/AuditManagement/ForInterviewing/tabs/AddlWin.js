...
var AddlWin = React.createClass({
    ...
    validateTab1() {//验证第一个tab页
        var BorrowingNeeds = this.refs.BorrowingNeeds;
        var me = this;
        var A = false;
        BorrowingNeeds.validateFields((errors, values) => {
            if (!!errors) {
    ...
    validateTab4() {//验证第4个tab页
        var RealEstate = this.refs.RealEstate;
        var MarriageInformation = this.refs.MarriageInformation
        var CreditInformat = this.refs.CreditInformat
        var RiskControl = this.refs.RiskControl
        var me = this;
        var B = false;
        var C = false;
        var D = false;
    ...
    handleOk() {
        var me = this;
        if (!this.validateTab1()) {//验证第一个TAB页
            me.handleTabClick("1");
            return
        } else {
            var RealEstate = this.refs.RealEstate;
            if (!RealEstate) {//判断第4个TAB是否有值
                me.handleTabClick("4");
                return;
            } else {
                    ...
                    if (!ProcessInformation) {//判断第5个TAB是否有值
                        ...
                        validation6((errors, values) => {//验证第5个TAB页
                            if (!!errors) {
                                ...
                                var housFaceTrial = this.refs.RealEstate.getFieldsValue();
                                var housMarriageInformation = this.refs.MarriageInformation.getFieldsValue();
                                var housCreditInformation = this.refs.CreditInformat.getFieldsValue();
                                var housControlInformation = this.refs.RiskControl.getFieldsValue();
                                ...
                                postdata.housCreditInformation = housCreditInformation;
                                postdata.housFaceTrial = housFaceTrial;
                                postdata.housMarriageInformation = housMarriageInformation;
                                postdata.housControlInformation = housControlInformation;
    ...
    render() {
        ...
        if (props.title == "面审" && state.activekey == 4) {
            modalBtns = [
                <button key="back" className="ant-btn" onClick={this.handleCancel}>返 回</button>,
        ...
        if (props.title == "面审" && state.activekey == 1) {
            modalBtns = [
                <button key="back" className="ant-btn" onClick={this.handleCancel}>返 回</button>,
                <button key="submit" className="ant-btn" onClick={this.handleSave1}>保存草稿</button>,
                <button key="button" className="ant-btn ant-btn-primary" disabled={!props.canEdit}  onClick={this.handleOk}>
                    提 交
                </button>
            ];
                        ...
                        <div style={{ position: "relative" }}>
                            <div className="navLine-wrap" onScroll={NavLineUtils._handleSpy.bind(this, this.navLineData1) }>
                                <div className="col-22 navLine-wrap-left" >
                                    <div id="b1">
                                        <h2>房产信息</h2>
                                        <HousingInformation ref="HousingInformation" canEdit={canEditBasicInfo} record={props.record} title={props.title} selectRecord={props.record}/>
                            ...
                            <div className="navLine-wrap" onScroll={NavLineUtils._handleSpy.bind(this, this.navLineData3) }>
                                <div className="col-22 navLine-wrap-left" >
                                    <div id="a1">
                                        ...
                                        <UploadPhoto style={{ marginLeft: 25 }} title="户口本" btype="HOUSEHOLD" canEdit={props.canEdit}  selectRecord={props.record}/>
                                    </div>
                                    ...
                                    <div id="a3" style={{ marginTop: "10px" }}>
                                        ...
                                        <UploadPhoto style={{ marginLeft: 25 }} title="征信资料" btype="CREDIT" canEdit={props.canEdit} selectRecord={props.record}/>
                                    </div>
                                    <div id="a4" style={{ marginTop: "10px" }}>
                                        <h2>风控信息</h2>
                                        <RiskControl ref="RiskControl" canEdit={props.canEdit} record={props.record}/>
                                        <UploadPhoto style={{ marginLeft: 25 }} title="风控单" btype="RISK" canEdit={props.canEdit} selectRecord={props.record}/>
                                    </div>
                                </div>
                                <div className="navLine-wrap-right" >
