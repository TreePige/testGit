  ...
  showModal(title, canEdit) {
    ...
    if (title == "查看") {
      Utils.ajaxData({
        ...
        callback: (result) => {
          this.refs.AddlWin.refs.SettlementMangWin.setFieldsValue(result.data.serviceVariables);
        }
      });
    }
    this.setState({
    ...
    return (
      <div className="block-panel">
        <div className="actionBtns" style={{ marginBottom: 16 }}>
          <button className="ant-btn"  disabled={!hasSelected} onClick={this.showModal.bind(this, '查看结算单', false) }>
            查看结算单
          </button>
        </div>
