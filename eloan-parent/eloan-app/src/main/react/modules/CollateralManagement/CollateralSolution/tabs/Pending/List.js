  ...
  showModal(title, canEdit) {
    ...
    if (title == "抵押登记") {
      Utils.ajaxData({
        ...
        callback: (result) => {
          this.refs.SolutionWin.refs.CollateralSolutionWin.setFieldsValue(result.data.housLowerCostBasicInfo);
        }
      });
    }
    this.setState({
    ...
    return (
      <div className="block-panel">
        <div className="actionBtns" style={{ marginBottom: 16 }}>
          <button className="ant-btn"  disabled={!hasSelected} onClick={this.showModal.bind(this, '押品出库', true) }>
            押品出库
          </button>
        </div>
